...
// --- Service Worker registrace (ponecháme) ---
...
function exportCurrent() {
  ...
  if (!user) { alert('Uživatel nenalezen.'); return; }
  const data = {
    app: 'Kubb Tracker',
    format: 1,
    exportedAt: new Date().toISOString(),
...
async function importJSONFile(file) {
  try {
    ...
    showToast(`Import hotov: +${addedRecords} záznamů, ${addedUsers} nových uživatelů${skippedDupes?`, přeskočeno duplicit: ${skippedDupes}`:''}.`, 3500);
  } catch (e) {
...
function makeEntrySignature(entry) {
  ...
  const base = {
    ...
    dnf: entry.dnf ?? null,
    training: Array.isArray(entry.training)
      ? entry.training.map(s => ({ hit: s.hit, throws: s.throws }))
      : null,
...
function normalizeEntry(e) {
  if (!e) return;
  if (e.type === '8m') {
    if (e.totalHit == null && Array.isArray(e.training)) {
      e.totalHit = e.training.reduce((a,c)=>a+(c.hit||0),0);
    }
    if (e.totalThrows == null && Array.isArray(e.training)) {
      e.totalThrows = e.training.reduce((a,c)=>a+(c.throws||0),0);
    }
    if (e.successRate == null && e.totalThrows > 0) {
      e.successRate = Math.round((e.totalHit / e.totalThrows) * 100);
    }
  }
  if (e.type === '8+2' && e.mode === 'classic') {
    if (e.total11 == null) {
      e.total11 = (e.in10 || 0) + (e.king ? 1 : 0);
    }
  }
}

// Hook na tlačítka export/import + přepínač období
document.addEventListener('DOMContentLoaded', () => {
  ...
  // přepínač období ve statistikách
...
function switchMode(mode) {
  currentMode = mode;
  if (mode === '8m') {
    mode8mDiv.style.display = '';
    mode82Div.style.display = 'none';
  } else {
    mode8mDiv.style.display = 'none';
    mode82Div.style.display = '';
  }
}
...
document.getElementById('end-training-btn').onclick = () => {
  ...
  if (userIndex === -1) { alert('Uživatel nenalezen.'); return; }
...
function switchM82Submode(sub) {
  m82Submode = sub;
  if (sub === 'classic') {
    m82ClassicDiv.style.display = '';
    m82UnlimitedDiv.style.display = 'none';
  } else {
    m82ClassicDiv.style.display = 'none';
    m82UnlimitedDiv.style.display = '';
  }
}
...
document.querySelectorAll('.m82c-chip').forEach(chip => {
  chip.addEventListener('click', () => {
    ...
    if (idx === -1) { alert('Uživatel nenalezen.'); return; }
...
function parsePageSize(val) {
  return (val === 'all') ? Infinity : parseInt(val || '30', 10);
}
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') closeModal('history-modal');
});

// ====== STATISTIKY ======

// pomocná: vyfiltruj historii podle zvoleného období
function filterHistoryByRange(history, range) {
  if (!Array.isArray(history)) return [];
  if (range === 'all') return history.slice(); // vše (nezkracovat)
  const now = Date.now();
...
function computeStatsFromHistory(history) {
  ...
  h.forEach(e => {
    if (e.type === '8m') {
      ...
      res.count['8m']++;
      res.m8m.sessions++;
      res.m8m.totalHits += hits;
      res.m8m.totalThrows += throws;
      ...
      if (e.dnf) {
        res.u82.dnf++;
        res.count['8p2Unlimited']++;
        res.u82.sessions++;
        pushLast(res.u82.last5, null);
      } else {
        ...
        if (p != null) {
          ...
          res.u82.avgPins += p;
          pushLast(res.u82.last5, p);
          res.u82.series.push(p);
        }
      }
    }
  });
  ...
  const uVals = h.filter(e => e.type==='8+2' && e.mode==='unlimited' && !e.dnf)
                 .map(e => e.pinsToClose);
...
function sparklineSVG(values, { min=null, max=null, invert=false } = {}) {
  ...
  const lastNorm = (last - lo) / span;
  const lastY = 5 + (invert ? lastNorm : (1 - lastNorm)) * 30;
...
function clearStatsView() {
  ...
  const note = document.getElementById('stats-note');
  if (root) {
    root.innerHTML = '<div style="opacity:.7;">Vyber uživatele, zobrazí se souhrny pro 8m, 8+2 Classic a 8+2 Unlimited.</div>';
  }
  if (note) note.textContent = '';
}
